...
export function LassoGroup(
    ...
    modeling,
    mouse, elementFactory, directEditing, graphicsFactory) {
  ...
  */
  var visuals = {
    ...
    remove: function(context) {
      ...
      if (context.groupVisual) {
        svgRemove(context.groupVisual);
      }
    }
  };

  function drawGroup(context) {
    const container = canvas.getActiveLayer(),
          bbox = context.bbox;

    const group = context.group = context.group = elementFactory.createShape({ type: 'bpmn:Group', height: bbox.height, width: bbox.width });
    context.groupVisual = graphicsFactory.drawShape(container, group);

    svgAttr(context.groupVisual, { x: bbox.x, y: bbox.y });
    svgAppend(container, context.groupVisual);
  }
